...
const Logo = ({ p }: { p: typeof partners[0] }) => {
  const base = "flex items-center gap-1.5 text-[#8B90A7] font-medium shrink-0";
  ...
  if (p.name === "Pinterest") return (
    <span className={`${base} text-sm`}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12c0 4.24 2.65 7.86 6.39 9.29-.09-.78-.17-1.98.04-2.83.18-.77 1.22-5.17 1.22-5.17s-.31-.63-.31-1.56c0-1.46.85-2.55 1.9-2.55.9 0 1.33.67 1.33 1.48 0 .9-.58 2.26-.87 3.51-.25 1.05.52 1.9 1.55 1.9 1.85 0 3.28-1.95 3.28-4.77 0-2.49-1.79-4.23-4.35-4.23-2.96 0-4.7 2.22-4.7 4.52 0 .9.35 1.85.78 2.37.09.1.1.19.07.3-.08.32-.25 1.05-.29 1.19-.04.19-.14.23-.32.14-1.25-.58-2.03-2.42-2.03-3.9 0-3.15 2.29-6.05 6.61-6.05 3.47 0 6.16 2.47 6.16 5.77 0 3.45-2.17 6.22-5.19 6.22-1.01 0-1.97-.53-2.3-1.15l-.62 2.33c-.23.87-.84 1.95-1.25 2.61.94.29 1.94.45 2.97.45 5.52 0 10-4.48 10-10S17.52 2 12 2z"/></svg>
      Pinterest
    </span>
  );
  if (p.name === "freepik") return (
    <span className={`${base} text-sm`}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="1.5"/><path d="M8 12h8M12 8v8" stroke="currentColor" strokeWidth="1.5"/></svg>
      freepik
    </span>
  );
  if (p.name === "Snapp!") return <span className={`${base} font-extrabold text-base`}>Snapp!</span>;
  if (p.name === "Uber") return <span className={`${base} font-bold text-lg`}>Uber</span>;
  return <span className={`${base} text-sm font-semibold`}>{p.name}</span>;
};
